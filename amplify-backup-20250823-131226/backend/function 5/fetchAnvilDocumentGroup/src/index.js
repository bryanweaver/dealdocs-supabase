...
exports.handler = async (event) => {
  ...
  try {
    ...
    const { documentGroupEid } = JSON.parse(event.body);

    console.log(`Fetching document group with EID: ${documentGroupEid}`);

    const response = await client.downloadDocuments(documentGroupEid);
    ...
    console.log("API final documents", documents);
    ...
    return {
      ...
      body: JSON.stringify({
        message: "Document group files retrieved successfully",
        documents,
      }),
    };
  } catch (error) {
